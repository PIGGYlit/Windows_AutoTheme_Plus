...
**Windows AutoTheme Plus** 是一个轻量级的 Windows 主题与壁纸自动切换工具。支持根据 Windows **夜灯（护眼模式）** 状态自动切换系统亮/暗主题和壁纸，也支持按固定时间定时切换。
...
- **跟随系统**：检测 Windows 夜灯（护眼模式）状态，夜灯开启时自动切深色主题/壁纸，关闭时切回浅色
- **定时切换**：支持手动设置时间段，到点自动切换亮/暗主题和壁纸
- **自定义壁纸**：绑定浅色/深色两套壁纸图片，切换时自动替换桌面壁纸
- **自定义主题**：支持绑定 `.theme` 主题文件，切换时自动应用指定主题
- **事件驱动**：夜灯状态变化即时响应，告别轮询
...
通过 Windows 注册表监听（`RegNotifyChangeKeyValue`）实时捕获夜灯状态变化，即时响应，无轮询开销。
